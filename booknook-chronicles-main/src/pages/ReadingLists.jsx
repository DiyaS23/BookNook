...
const ReadingLists = () => {
  const { authAxios, user, isAdmin } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const usersResponse = await authAxios.get(`${API_BASE_URL}/users`);
        ...
        // Handle errors specifically for the protected users endpoint
        const isUserEndpointError = error.config.url.includes('/users');
        const status = error.response?.status;
        
        let errorMessage = "Failed to fetch data";
        
        if (isUserEndpointError && status === 403) {
             errorMessage = "Access Denied: Admin role required to fetch the users list for the filter.";
        } else if (isUserEndpointError && status === 401) {
             errorMessage = "Login required to fetch the user filter list.";
        }
  ...
  const handleDeleteList = async (listId) => {
    if (!user) {
        toast({ title: "Error", description: "You must be logged in to delete lists.", variant: "destructive" });
        return;
    }
    
    try {
      // ⬅️ Call the DELETE endpoint
      await authAxios.delete(`${API_BASE_URL}/readinglists/${listId}`);
      ...
      const errorMsg = error.response?.status === 403 ? "Access Denied: Only the list owner or Admin can delete." : "Failed to delete reading list.";
      toast({
        title: "Error",
          ...
          <Card key={list.id} className="card-elegant group">
            ...
            <CardContent>
              ...
              <div className="flex gap-2">
                ...
                {(isAdmin || (user && list.user?.id === user.id)) && (
                    <Button
                      variant="destructive"
                      size="sm"
