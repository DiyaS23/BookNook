...
const EditUser = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
       if (!user || (!isAdmin && user.id.toString() !== id)) {
           toast({ title: "Error", description: "Access Denied: Not authorized to edit this profile.", variant: "destructive" });
           navigate("/users");
           setLoading(false);
           return;
      }
    ...
    fetchUser();
  }, [id, toast, navigate, user, isAdmin]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await authAxios.put(`${API_BASE_URL}/users/${id}`, userData);
      toast({
        title: "Success",
      ...
      const errorMsg = error.response?.status === 403 ? "Access Denied: Not authorized to update this profile." : "Failed to update user. Please try again.";
      toast({
        title: "Error",
